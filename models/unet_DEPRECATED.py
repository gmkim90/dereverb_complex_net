...
def pad2d_as(x1, x2):
    ...
    return Func.pad(x1, (0, diffW, 0, diffH)) # (L,R,T,B)
...
class Unet(nn.Module):
    def __init__(self, cfg, loss_type = 'cossim_time', nMic = 1, reverb_frame = 1, use_depthwise=False, nFreq=0, use_bn=True, input_type ='complex', ds_rate = 1):
    ...
    def IMF_given_td_direct_only(self, xr, xi, td_Tensor):
        # xr, xr: NxMxFxT
        eps = 1e-10
        fs = 16000
        N, M, F, T = xr.size()
        yr, yi = torch.FloatTensor(N, M-1, F, T).cuda(), torch.FloatTensor(N, M-1, F, T).cuda()
        mic_magnitude = torch.sqrt(xr*xr + xi*xi + eps)
        linear_relative_freq = torch.linspace(0, F-1, F).cuda()/(2*(F-1))*2*math.pi*(-1)*fs
        #mic_phase = torch.atan2(xi, xr)

        yi = linear_relative_freq.view(1, 1, F, 1).expand(N, M-1, F, T) * td_Tensor.cuda().view(N, M-1, 1, 1).expand(N, M-1,F,T)
    ...
    def IMF_given_td_unwrap(self, xr, xi, td_Tensor):
        # xr, xr: NxMxFxT
        eps = 1e-10
        N, M, F, T = xr.size()
        yr, yi = torch.FloatTensor(N, M-1, F, T).cuda(), torch.FloatTensor(N, M-1, F, T).cuda()
        ...
        direct_path_phase = linear_relative_freq.view(1, 1, F, 1).expand(N, M-1, F, T) * td_Tensor.cuda().view(N, M-1, 1, 1).expand(N, M-1,F,T)

        TWOPI = math.pi*2
        for m in range(M-1):
            yr[:, m] = torch.log(mic_magnitude[:, m+1] + eps) - torch.log(mic_magnitude[:, 0] + eps)
            yi[:, m] = mod(mic_phase[:, m + 1] - mic_phase[:, 0] + math.pi, TWOPI) -  math.pi

        diff = direct_path_phase - yi
        unwrap_integer = torch.round(diff/TWOPI)

        yi = yi + TWOPI*unwrap_integer
    ...
    def forward(self, xr, xi, td=None):
        ...
        skips = list()
        if(self.input_type == 'IMF_given_td_direct_only'):
            xr, xi = self.IMF_given_td_direct_only(xr, xi, td)
        elif(self.input_type == 'IMF_given_td_unwrap'):
            xr, xi = self.IMF_given_td_unwrap(xr, xi, td)
        elif(self.input_type == 'complex_ratio'):
